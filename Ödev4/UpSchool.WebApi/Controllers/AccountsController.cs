...
public class AccountsController : ControllerBase
{
    ...
    [HttpGet]
    public IActionResult GetAll(bool isAscending, string? searchKeyword)      //Bir searchKeyword gelebilir de gelmeyebilir de 
    {
        IQueryable<Account> accountsQuery = _dbContext.Accounts.AsQueryable();

        if (!string.IsNullOrEmpty(searchKeyword))
            accountsQuery =
                accountsQuery.Where(x => x.Title.Contains(searchKeyword) || x.UserName.Contains(searchKeyword));

        accountsQuery = isAscending 
            ? accountsQuery.OrderBy(x => x.Title)       // Küçükten büyüğe sırala
            : accountsQuery.OrderByDescending(x => x.Title);        // Büyükten küçüğe sırala

        var accounts = accountsQuery.ToList();
        
        var accountDtos = accounts.Select(account => AccountDto.MapFromAccount(account)).ToList();      // select her biri için demek. Anonim fonksiyon
    ...
    public IActionResult GetById(Guid id)
    {
        ...
        return Ok(AccountDto.MapFromAccount(account));      //account olarak değil accountdto olarak dönmemiz lazım.
    }
    ...
    {
        // var accountIndex = _accounts.FindIndex(x => x.Id == accountEditDto.Id);        // O id de kayıt varsa getirecek yoksa null döndürecek.
    ...
    {
        var account = _dbContext.Accounts.FirstOrDefault(x => x.Id == id);        // O id de kayıt varsa getirecek yoksa null döndürecek.
